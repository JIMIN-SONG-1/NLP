...
def count_words(input_text):
    ...
    words = word_tokenize(input_text.lower())  # 모든 단어를 소문자로 변환하여 토큰화
    output_dict = Counter(words) # Counter를 사용하여 단어 개수를 효율적으로 계산
    return output_dict
...
def remove_stopwords(input_dict):
    ...
    output_dict = {word: count for word, count in input_dict.items() if word not in stop_words}
    return output_dict
...
def remove_less_freq(input_dict, lower_bound=10):
    ...
    output_dict = {word: count for word, count in input_dict.items() if count >= lower_bound}
    return output_dict


def main():
    # 데이터 파일인 'text8_1m_part_aa.txt' 파일을 불러옵니다.
    with codecs.open("/Users/jamie/Desktop/NLP-practice-main/practice2/text8_1m_part_aa.txt", "r", "utf-8") as html_f:
